							...
							for h in `python $FLOAT_RANGE_DIR $h_start $h_stop $h_spacing`; do
								...
								if [ "$overwrite" = "No" ]; then
									if [ `ls -1 "$SOL_DIR/Phase_difference_$phase/T_$T/Rashba_${alpha_1}_${alpha_2}_${alpha_3}_${alpha_4}/h_${h}_angle_${h_angle}/" |wc -l` != $matsubara_sum ]; then
										python src/supercurrent.py $phase $T $alpha_1 $alpha_2 $alpha_3 $alpha_4 $h $h_angle $delta $phase_spacing $T_spacing $alpha1_spacing $alpha2_spacing $alpha3_spacing $alpha4_spacing $h_spacing $h_angle_spacing $SOL_DIR $RES_DIR $matsubara_sum $tolerance
									fi
