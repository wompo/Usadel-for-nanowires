...
# The maximum energy used in the calculation. If this is zero and overwrite is "No" all parameters are skipped!
max_energy=30
						...
						for h in `python $FLOAT_RANGE_DIR $h_start $h_stop $h_spacing`; do
							...
							if [ "$overwrite" = "No" ]; then
								if [ `ls -1 "$SOL_DIR/Phase_difference_$phase/Rashba_${alpha_1}_${alpha_2}_${alpha_3}_${alpha_4}/h_${h}_angle_${h_angle}/" |wc -l` != $(bc <<< "$max_energy / $energy_res + 1") ]; then
									python $PARENT_DIR/src/DOS.py $phase $alpha_1 $alpha_2 $alpha_3 $alpha_4 $h $h_angle $delta $phase_spacing $alpha1_spacing $alpha2_spacing $alpha3_spacing $alpha4_spacing $h_spacing $h_angle_spacing $SOL_DIR $RES_DIR $max_energy $energy_res $tolerance
								fi
